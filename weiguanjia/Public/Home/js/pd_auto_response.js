$(document).ready(function() {
    ...
    $('.modal-content').find('.modal-footer').find('[data-type=ensure]').click(function() {
        if(typeof where!="undefined"){
            $data=$('.modal-content').find('[data-selected=selected]').clone().removeClass().addClass('col-xs-12 column');
            if($('.modal-content').find('[data-selected=selected]').parent('.video_list').length){
                $data.find('.data_box_video').children().removeClass().addClass('col-xs-12 column');
            }
        }else{
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    ...
                    if ($result == 'error') {
                        ...
                        switch ($type) {
                            case 'image':
                                $ele = $('<div class="col-xs-3 column cell" data-type="data_box_cell"><div class="col-xs-12 column data_box_img"><img src="openImage?url=' + $json.url + '" height="117" width="100%" data-id="' + $json.media_id + '"/><p class="text-center">' + filesObj.name.substr(0, 15) + '</p></div></div>');
                                $ele.css('opacity', '1.0');
                                break;
                            case 'voice':
        ...
        if(data=='error'){
            ...
            if($len){
                for($i=0;$i<$len;$i+=1){
                    switch($type){
                        case 'image':
                            $ele='<div class="col-xs-3 column cell" data-type="data_box_cell" onclick="add_select_animate(this)"><div class="col-xs-12 column data_box_img"><img src="openImage?url='+$list.item[$i].url+'" height="117" width="100%" data-id="'+$list.item[$i].media_id+'"/><p class="text-center">'+$list.item[$i].name.substr($list.item[$i].name.lastIndexOf('/')+1,15)+'</p></div></div>';
                        ...
                        case 'news':
                            if($list.item[$i].content.news_item[1]){    //多图文素材
                                ...
                                for($k=0;$k<$childLen;$k+=1){
                                    if($k==0){
                                        $childEle='<div class="first data_box"><img src="openImage?url='+auto_thumb_url($list.item[$i].content.news_item[$k].thumb_url)+'" width="100%" height="120px"> <h5 class="sm">'+$list.item[$i].content.news_item[$k].title.substr(0,15)+'</h5></div>';
                                        $ele.find('.news_body').append($childEle);
                                    }else{
                                        $childEle='<div class="col-xs-12 column data_box"><div class="col-xs-8 column"><h5>'+$list.item[$i].content.news_item[$k].title.substr(0,15)+'</h5></div><div class="col-xs-4 column"><img width="100%" height="70" src="openImage?url='+auto_thumb_url($list.item[$i].content.news_item[$k].thumb_url)+'"></div></div>'
                                        $ele.find('.news_body').append($childEle);
                                    }
                                }
                            }else{  //单图文素材
                                $date=new Date($list.item[$i].update_time*1000);
                                $ele='<div class="col-xs-4 column cell" onclick="add_select_animate(this)" data-type="data_box_cell"><div class="col-xs-12 column news_box"><div class="news_header_single"><h5>'+ $date.getFullYear()+'年'+$date.getMonth()+'月'+$date.getDate()+'日'+'</h5></div> <div class="col-xs-12 column news_body"> <div class="col-xs-12 column data_box"> <p>'+$list.item[$i].content.news_item[0].title.substr(0,15)+'</p><img src="openImage?url='+auto_thumb_url($list.item[$i].content.news_item[0].thumb_url)+'" width="100%" height="150px"><p>'+$list.item[$i].content.news_item[0].digest+'</p></div></div><div class="col-xs-12 column news_footer" data-id="'+$list.item[$i].media_id+'"></div></div></div>';
                            }
